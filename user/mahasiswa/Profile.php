...
class Profile {
    ...
    //     $profilePic = $_SESSION['profile_picture'] ?? '../../assets/icon/user.png';
    //     echo '<img id="profile-pic" src="' . htmlspecialchars($profilePic) . '" alt="profile" width="150px" height="150px">';
    // }
    ...
    public function updateNIM($nim) {
        error_log("updateNIM called");
        $query = "UPDATE p_mahasiswa SET nim = ? WHERE user_id = ?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("si", $nim, $this->user_id);

        if ($stmt->execute()) {
            error_log("NIM update success for user_id: " . $this->user_id);
            return true;
        } else {
            error_log("NIM update failed for user_id: " . $this->user_id . " with error: " . $stmt->error);
            return false;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($action) {
        case 'updateProfile':
            ...
            // if (isset($_POST['country'])) $profileData['country'] = $_POST['country'];
        ...
        //     if (isset($_POST['nim'])) {
        //         $nim = $_POST['nim'];
        //         $success = $profile->updateNIM($nim);
        //         echo json_encode(['success' => $success]);
        //     } else {
        //         echo json_encode(['success' => false, 'message' => 'NIM not provided.']);
        //     }
        //     break;
        ...
        //         $success = $profile->updatePassword($password);
        //         echo json_encode(['success' => $success]);
        //     } else {
...
<body>
    <div class="wrapper">
        ...
        <div class="modal fade" id="updateAlamatModal" tabindex="-1" aria-labelledby="editalamat" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
                <div class="modal-content">
                    <form id="updateAlamatForm" method="post">
                        <div class="modal-header">
        ...
        $(document).ready(function() {
            ...
            $('#formEditPicture').submit(function(event) {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        //     closeModal('#editpicture');
                        //     reloadPage();
                        // } else {
                        //     alert('Profile picture update failed.', 'danger');
                        // }
                    // },
                    // error: function() {
                    //     hideLoading();
            ...
            $('#updateAlamatForm').submit(function(event) {
                ...
                $.ajax({
                    ...
                    //         closeModal('#updateAlamatModal');
                    //         reloadPage();
                    //     } else {
                    //         alert('Address update failed.','danger');
                    //     }
                    // },
                    // error: function() {
                    //     // hideLoading();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (res.success) {
                            ...
                            closeModal('#editprofil');
                            reloadPage();
                        } else {
                            alert('Profile update failed.', 'danger');
                        }
                    },
                    error: function() {
                        hideLoading();
